...
/**
 * @Description : BasicFragment
 * @Copyright   : GL. All Rights Reserved
 * @Company     : 深圳市国立数码动画有限公司
 * @author      : 叶跃丰
 * @version     : 1.0
 * Create Date  : 2014-8-11 下午2:59:19
 */
public abstract class BasicFragment extends BaseFragment implements OnClickListener {
	...
	protected void initLoadView() {
        ...
        mIvEmpty=getView(R.id.iv_empty);
	...
	protected boolean isNetConnected() {
		return NetworkUtil.isNetworkConnected(this.getActivity());
	}
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case GlobalMessageType.ActivityReqCode.REQ_GET_PHOTO:
			if (resultCode == Activity.RESULT_OK) {
				...
				String file = null;
				if (ImageExUtils.isKitKat()) {
					file = ImageExUtils.getPath(mContext, uri);
					bitmap = ImageExUtils.getBitmap(file);
				} else {
					bitmap = ImageUtils.getThumbnailImage(mContext, uri);
					file = ImageUtils.getImagePath(mContext, uri);
				}
	...
	protected void browseImage(List<ImageInfoModel> list) {
		Intent intent = new Intent(mContext, BrowsePictureActivity.class);
	...
	protected void showSubmitDialog(String message) {
		closeSubmitDialog();
		mSubmitDialog = new CommonProgressDialog(mContext, StringUtils.isNotEmpty(message) ? message : getString(R.string.do_request_ing));
		mSubmitDialog.show();
	}
