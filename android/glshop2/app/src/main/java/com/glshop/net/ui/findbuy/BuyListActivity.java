...
public class BuyListActivity extends BasicFragmentActivity implements BaseMenuDialog.IMenuCallback {
	...
	private void resetFilterStatus() {
		// 1.Reset cache info
		if (mBuyFilterInfoV2 != null) {
			if (BeanUtils.isNotEmpty(mBuyFilterInfoV2.productIdList)) {
				SysCfgUtils.resetProductFilterList(this);
			}
			if (BeanUtils.isNotEmpty(mBuyFilterInfoV2.provinceCodeList) || BeanUtils.isNotEmpty(mBuyFilterInfoV2.districtCodeList)) {
				SysCfgUtils.resetAreaFilterList(this);
			}
		}

		// 2.Renew Filter info
		mBuyFilterInfoV2 = new BuyFilterInfoModelV2();
		updateBuyList(false);
	}

	private void updateBuyList(boolean isFilterMode) {
		mFragmentBuyer.updateBuyFilterInfo(mBuyFilterInfoV2, isFilterMode);
		mFragmentSeller.updateBuyFilterInfo(mBuyFilterInfoV2, isFilterMode);
	}
