...
public class DBHelper extends SQLiteOpenHelper {
	...
	public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
		Logger.e(TAG, "DB onUpgrade: oldVersion = " + oldVersion + ", newVersion = " + newVersion);
	...
	/**
	 * 版本2: 数据库中表中新增了order字段，用于货物规格排序。
	 * @param db
	 */
	protected void upgradeVersion2(SQLiteDatabase db) {
		db.execSQL("DROP TABLE IF EXISTS " + DBConstants.TableSyscfgType.TABLE_NAME);
		db.execSQL("DROP TABLE IF EXISTS " + DBConstants.TableSyscfgData.TABLE_NAME);
		db.execSQL("DROP TABLE IF EXISTS " + DBConstants.TableProductData.TABLE_NAME);
		db.execSQL("DROP TABLE IF EXISTS " + DBConstants.TableProductPropData.TABLE_NAME);
		db.execSQL(DBConstants.TableSyscfgType.CREATE_TABLE_SQL);
		db.execSQL(DBConstants.TableSyscfgData.CREATE_TABLE_SQL);
		db.execSQL(DBConstants.TableProductData.CREATE_TABLE_SQL);
		db.execSQL(DBConstants.TableProductPropData.CREATE_TABLE_SQL);
	...
	/**
	 * 版本3: 数据库中表中修改TableProductData、TableProductPropData中Text数据类型为Float类型
	 * @param db
	 */
	 ...
	 * @param db
	 */
	protected void upgradeVersion4(SQLiteDatabase db) {
		// TODO
	}

	/**
	 * 版本5: 清除数据库中TableAreaCfg表中数据
	 * @param db
	 */
	protected void upgradeVersion5(SQLiteDatabase db) {
		db.execSQL("DROP TABLE IF EXISTS " + DBConstants.TableAreaCfg.TABLE_NAME);
		db.execSQL(DBConstants.TableAreaCfg.CREATE_TABLE_SQL);
	}
