...
public class UfmContractListAdapter extends BasicAdapter<ContractSummaryInfoModel> implements OnClickListener {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (model.lifeCycle == ContractLifeCycle.TIMEOUT_FINISHED || model.lifeCycle == ContractLifeCycle.DRAFTING_CANCEL) { // 起草超时结束
			mTvWaitTime.setText("15分钟有效期限已失效");
			mTvWaitTime.setTextColor(mContext.getResources().getColor(R.color.gray));
		} else {
			if (StringUtils.isNotEmpty(model.draftExpireTime)) {
				mTvWaitTime.setText(DateUtils.getContractSignWaitTime(model.draftExpireTime));
			} else {
				mTvWaitTime.setText("0小时0分");
			}
		...
		if (ContractUtil.isValid(model)) {
			...
			if (model.lifeCycle == ContractLifeCycle.TIMEOUT_FINISHED) { // 起草超时结束
				...
				mTvStatusDetail.setText(getTimeoutStatus(model));
		...
		if (ContractUtil.isValid(model)) {
			...
			mTvAmountUnit.setTextColor(mContext.getResources().getColor(R.color.red));
			mTvPriceUnit.setTextColor(mContext.getResources().getColor(R.color.red));
			...
			mTvAmountUnit.setTextColor(mContext.getResources().getColor(R.color.black));
			mTvPriceUnit.setTextColor(mContext.getResources().getColor(R.color.black));
	...
	private String getTimeoutStatus(ContractSummaryInfoModel info) {
		String status = "";
		ContractDraftStatus myStatus;
		ContractDraftStatus otherStatus;
		if (info.buyType == BuyType.BUYER) {
		...
		if (myStatus == ContractDraftStatus.NOTHING && otherStatus == ContractDraftStatus.NOTHING) {
			status = mContext.getString(R.string.ufm_contract_invalide_1);
		} else if (myStatus == ContractDraftStatus.NOTHING && otherStatus == ContractDraftStatus.CONFIRMED) {
			status = mContext.getString(R.string.ufm_contract_invalide_3);
		} else if (myStatus == ContractDraftStatus.CONFIRMED && otherStatus == ContractDraftStatus.NOTHING) {
			status = mContext.getString(R.string.ufm_contract_invalide_4);
		}
		return status;
	}
