...
public class UserLogic extends BasicLogic implements IUserLogic {
	...
	@Override
	public void getImgVerifyCode(final String invoker, String file, String deviceId) {
		GetImgVerifyCodeReq req = new GetImgVerifyCodeReq(invoker, new IReturnCallback<GetImgVerifyCodeResult>() {

			@Override
			public void onReturn(Object invokerObj, ResponseEvent event, GetImgVerifyCodeResult result) {
				if (ResponseEvent.isFinish(event)) {
	...
	@Override
	public void getSmsVerifyCode(final String invoker, String account, String sendType) {
	...
	@Override
	public void getSmsVerifyCode(final String invoker, String deviceId, String account, String imgVerifyCode) {
	...
	public void validImgVerfiyCode(String account, String deviceId, String verifyCode) {
		ValidImgVerifyCodeReq req = new ValidImgVerifyCodeReq(this, new IReturnCallback<CommonResult>() {
			...
			public void onReturn(Object invoker, ResponseEvent event, CommonResult result) {
				if (ResponseEvent.isFinish(event)) {
					...
					message.obj = getOprRespInfo(result);
					if (result.isSuccess()) {
						message.what = UserMessageType.MSG_VALID_IMG_VERFIYCODE_SUCCESS;
					} else {
						message.what = UserMessageType.MSG_VALID_IMG_VERFIYCODE_FAILED;
					}
					sendMessage(message);
				}
			}
	...
	public void checkUser(String account) {
		CheckUserReq req = new CheckUserReq(this, new IReturnCallback<CommonResult>() {
			...
			public void onReturn(Object invoker, ResponseEvent event, CommonResult result) {
				if (ResponseEvent.isFinish(event)) {
					...
					message.obj = getOprRespInfo(result);
					if (result.isSuccess()) {
						message.what = UserMessageType.MSG_CHECK_REG_USER_SUCCESS;
					} else {
						message.what = UserMessageType.MSG_CHECK_REG_USER_FAILED;
					}
					sendMessage(message);
				}
			}
	...
	public boolean autoLogin(boolean isRefreshToken) {
		...
		if (isTokenValidate()) {
			...
			if (PlatformConfig.getBoolean(GlobalConstants.SPKey.IS_REMEMBER_USER_PWD, false)) {
				...
				if (isRefreshToken) {
					refreshToken(GlobalConfig.getInstance().getUserAccount(), getClientID(), PlatformConfig.getString(PlatformConfig.USER_TOKEN), false);
				}
				return true;
			} else {
				// 用户不记住密码，则不自动登录
			...
			if (StringUtils.isNotEmpty(token) && isRefreshToken) {
				refreshToken(GlobalConfig.getInstance().getUserAccount(), getClientID(), token, true);
			}
			return false;
		}
	}
