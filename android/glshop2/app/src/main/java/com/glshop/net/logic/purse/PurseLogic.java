...
public class PurseLogic extends BasicLogic implements IPurseLogic {
	...
	@Override
	public void getPayTradeNo(final String invoker, PurseType purseType, double money) {
		GetTradeNoReq req = new GetTradeNoReq(this, new IReturnCallback<GetTradeNoInfoResult>() {

			@Override
			public void onReturn(Object invokerObj, ResponseEvent event, GetTradeNoInfoResult result) {
				if (ResponseEvent.isFinish(event)) {
					Logger.i(TAG, "GetPayTradeNoResult = " + result.toString());
					Message message = new Message();
		...
		req.purseType = purseType;
		req.money = money;
		req.exec();
	}

	@Override
	public void rptPayResult(final String invoker, String oid, PayResultType result) {
		RptPayResultReq req = new RptPayResultReq(this, new IReturnCallback<CommonResult>() {
