...
public class GLApplication extends BaseApplication {
	...
	public void onCreate() {
		...
		// 初始化日志
		Logger.initLogger(Logger.WARN, true, GlobalConstants.AppDirConstants.LOG);
		// 异常统一处理
	...
	private void initAllLogics() {
		...
		LogicFactory.registerLogic(IAdvertisingLogic.class,new AdvertisingLogic(mContext));
	}
	...
	private void importAreaCfg() {
		ISysCfgLogic mSysCfgLogic = LogicFactory.getLogicByClass(ISysCfgLogic.class);
		boolean isImported = PlatformConfig.getBoolean(GlobalConstants.SPKey.IS_IMPORTED_PORT_CFG);
		if (!isImported) {
			if (mSysCfgLogic != null) {
	...
	private void weixininit(){
		...
		api = WXAPIFactory.createWXAPI(mContext, GlobalConstants.WEIXIN_APP_ID, true);
		// 将该app注册到微信
