...
public class BuyInfoActivity extends BasicActivity {
	...
	private void initView() {
		...
		mBtnWantToDeal = getView(R.id.btn_want_to_deal);
		mBtnModifyPub = getView(R.id.btn_modify_pub);
		mBtnUndoPub = getView(R.id.btn_undo_pub);
		mBtnRepub = getView(R.id.btn_repub);
		mBtnViewContract = getView(R.id.btn_view_contract);

		mBtnDeletePub = getView(R.id.btn_commmon_action);
		mBtnDeletePub.setImageResource(R.drawable.selector_btn_delete);
		mBtnDeletePub.setOnClickListener(this);
		...
		mItemProfileTitle = getView(R.id.tv_item_profile_title);
		mItemViewEvaList = getView(R.id.tv_view_company_evluations);
	...
	private void initData() {
		...
		// 更新买卖类型标题
		BuyType buyType = BuyType.convert(getIntent().getIntExtra(GlobalAction.BuyAction.EXTRA_KEY_BUY_INFO_TYPE, BuyType.BUYER.toValue()));
	...
	private void updateUI() {
		...
		mItemAmount.setContentText(StringUtils.getDefaultNumber(mBuyInfo.tradeAmount));
		mItemUnitPrice.setContentText(StringUtils.getDefaultNumber(mBuyInfo.unitPrice));
		// 显示单位
	...
	private void updateBuyStatus() {
		String statusTimestamp = DateUtils.convertDate2String(DateUtils.COMMON_DATE_FORMAT, DateUtils.CONTRACT_DATETIME_FORMAT_V2, mBuyInfo.tradeUpdateDate);
		if (viewType == ViewBuyInfoType.FINDBUY) {
	...
	private void updateActionBar() {
		...
		if (viewType == ViewBuyInfoType.FINDBUY) {
			if (StringUtils.isNotEmpty(mBuyInfo.companyId) && mBuyInfo.companyId.equals(getCompanyId())) {
				// 自己发布的供求，不显示想交易
				mBtnWantToDeal.setVisibility(View.GONE);
			} else {
				mBtnWantToDeal.setVisibility(View.VISIBLE);
				if (mBuyInfo.isClicked) {
					mBtnWantToDeal.setEnabled(false);
					mBtnWantToDeal.setText(getString(R.string.clicked_want_to_deal));
				} else {
					mBtnWantToDeal.setEnabled(true);
				}
			}
		} else {
			mBtnWantToDeal.setVisibility(View.GONE);
		...
		// 取消发布
		mBtnUndoPub.setVisibility(viewType == ViewBuyInfoType.MYBUY && mBuyInfo.buyStatus == BuyStatus.VALID ? View.VISIBLE : View.GONE);
		// 修改发布
		mBtnModifyPub.setVisibility(viewType == ViewBuyInfoType.MYBUY && mBuyInfo.buyStatus == BuyStatus.VALID ? View.VISIBLE : View.GONE);

		// 重新发布
		if (viewType == ViewBuyInfoType.MYBUY) {
			if (mBuyInfo.buyStatus == BuyStatus.INVALID_EXPIRE || mBuyInfo.buyStatus == BuyStatus.INVALID_CREATED_CONTRACT || mBuyInfo.buyStatus == BuyStatus.INVALID_UNPASS_REVIEW
					|| mBuyInfo.buyStatus == BuyStatus.INVALID_CANCELED) {
				mBtnRepub.setVisibility(View.VISIBLE);
				...
				if (mBuyInfo.contractStatus != null && mBuyInfo.contractStatus == ContractStatusTypeV2.FINISHED && StringUtils.isNotEmpty(mBuyInfo.contractId)) {
					...
					mBtnRepub.setVisibility(View.GONE);
				}
			} else {
				mBtnRepub.setVisibility(View.GONE);
			}
		...
		if (viewType == ViewBuyInfoType.MYBUY) {
			if (mBuyInfo.buyStatus == BuyStatus.INVALID_CREATED_CONTRACT) {
				if (mBuyInfo.contractStatus != null && StringUtils.isNotEmpty(mBuyInfo.contractId)) {
					mBtnViewContract.setVisibility(View.VISIBLE);
				} else {
					mBtnViewContract.setVisibility(View.GONE);
				}
			} else {
				mBtnViewContract.setVisibility(View.GONE);
			}
		...
		// 删除供求
		mBtnDeletePub.setVisibility(viewType == ViewBuyInfoType.MYBUY && mBuyInfo.buyStatus != BuyStatus.VALID ? View.VISIBLE : View.GONE);
	}
	...
	private void updateProductInfo() {
		if (mBuyInfo != null) {
			...
			// 颜色&产地
			if (StringUtils.isNotEmpty(mBuyInfo.productColor)) {
			...
			if (StringUtils.isNotEmpty(mBuyInfo.productArea)) {
	...
	private void initPropInfo(BuyTextItemView item, ProductPropInfoModel info) {
		if (info != null) {
			if (StringUtils.isNotEmpty(info.mRealSize)) {
				item.setContentText(StringUtils.getDefaultNumber(info.mRealSize));
			} else {
				item.setContentText(getString(R.string.data_no_input));
				item.setContentColor(getResources().getColor(R.color.gray));
			}
		} else {
			if (item != null) {
				item.setContentText(getString(R.string.data_no_input));
	...
	private void updateDischargeAddrInfo() {
		if (mBuyInfo != null && mBuyInfo.addrInfo != null) {
			...
			mItemPortWaterDepth.setContentText(String.valueOf(addrInfo.uploadPortWaterDepth));
			mItemPortShipTon.setContentText(String.valueOf(addrInfo.shippingTon));
	...
	private void updateCompanyInfo() {
		...
		if (mBuyInfo.buyType == BuyType.BUYER) {
			mItemProfileTitle.setText(getString(R.string.buyer_profile_info));
			mItemViewEvaList.setText(getString(R.string.view_buyer_evluations));
		} else {
			mItemProfileTitle.setText(getString(R.string.seller_profile_info));
			mItemViewEvaList.setText(getString(R.string.view_seller_evluations));
		}
	...
	protected void handleStateMessage(Message message) {
		...
		case GlobalMessageType.BuyMessageType.MSG_UNDO_PUB_BUY_INFO_FAILED:
			...
			break;
		case GlobalMessageType.BuyMessageType.MSG_DELETE_PUB_BUY_INFO_SUCCESS:
			onDeletePubSuccess(respInfo);
			break;
		case GlobalMessageType.BuyMessageType.MSG_DELETE_PUB_BUY_INFO_FAILED:
			onDeletePubFailed(respInfo);
			break;
		case GlobalMessageType.BuyMessageType.MSG_REFRESH_MY_BUY_INFO:
	...
	private void onDeletePubSuccess(RespInfo respInfo) {
		...
		TipInfoModel tipInfo = new TipInfoModel();
		tipInfo.operatorTipTitle = getString(R.string.my_buy);
		tipInfo.operatorTipTypeTitle = getString(R.string.operator_tips_pub_delete_title);
		tipInfo.operatorTipContent = getString(R.string.operator_tips_pub_delete_content);
		tipInfo.operatorTipActionText1 = getString(R.string.operator_tips_pub_cancel_action_text);
		tipInfo.operatorTipActionClass1 = MainActivity.class;
		tipInfo.operatorTipAction1 = GlobalAction.TipsAction.ACTION_VIEW_MY_BUY;
		intent.putExtra(GlobalAction.TipsAction.EXTRA_KEY_TIPS_INFO, tipInfo);
	...
	private void onRefreshInfo(RespInfo respInfo) {
		if (viewType == ViewBuyInfoType.MYBUY) {
			if (isCurrentActivity() && mBuyInfo != null && respInfo != null) {
				String newBuyId = respInfo.strArg1;
				Logger.e(TAG, "CurBuyId = " + mBuyInfo.buyId + ", NewBuyId = " + newBuyId);
				if (isSameBuyInfo(mBuyInfo.buyId, newBuyId)) {
					Logger.e(TAG, "Buy id is match, so refresh the current buy page!");
					Intent intent = new Intent(this, BuyInfoActivity.class);
					intent.putExtra(GlobalAction.BuyAction.EXTRA_KEY_BUY_ID, newBuyId);
					intent.putExtra(GlobalAction.BuyAction.EXTRA_KEY_VIEW_BUY_INFO_TYPE, ViewBuyInfoType.MYBUY.toValue());
					intent.putExtra(GlobalAction.BuyAction.EXTRA_KEY_BUY_INFO_TYPE, mBuyInfo.buyType.toValue());
					startActivity(intent);
					finish();
					return;
				} else {
	...
	private boolean isSameBuyInfo(String oldBuyId, String newBuyId) {
		return StringUtils.isNotEmpty(oldBuyId) && StringUtils.isNotEmpty(newBuyId) && oldBuyId.equalsIgnoreCase(newBuyId);
	}
	...
	private void showCancelDialog() {
		...
		mCancelDialog.setContent(getString(R.string.cancel_pub_warning_tips));
	...
	private void showDeleteDialog() {
		...
		mDeleteDialog.setContent(getString(R.string.delete_pub_warning_tips));
	...
	private void deletePub() {
		showSubmitDialog(getString(R.string.submiting_request));
		mBuyLogic.deletePubBuyInfo(mBuyInfo.buyId);
	}
