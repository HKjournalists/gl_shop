...
public class BuyLogic extends BasicLogic implements IBuyLogic {
	...
	@Override
	public void getBuys(BuyFilterInfoModelV2 filterInfo, final boolean isFilterMode, final BuyType type, int pageIndex, int pageSize, final DataReqType reqType) {
		GetBuysReq req = new GetBuysReq(this, new IReturnCallback<GetBuysResult>() {
			...
			public void onReturn(Object invoker, ResponseEvent event, GetBuysResult result) {
				if (ResponseEvent.isFinish(event)) {
					...
					respInfo.strArg1 = String.valueOf(isFilterMode);
					message.obj = respInfo;
					if (result.isSuccess()) {
	...
	@Override
	public void deletePubBuyInfo(String buyId) {
		DeletePubBuyInfoReq req = new DeletePubBuyInfoReq(this, new IReturnCallback<CommonResult>() {

			@Override
			public void onReturn(Object invoker, ResponseEvent event, CommonResult result) {
				if (ResponseEvent.isFinish(event)) {
					Logger.i(TAG, "DeletePubBuyResult = " + result.toString());
					Message message = new Message();
					message.obj = getOprRespInfo(result);
					if (result.isSuccess()) {
						message.what = BuyMessageType.MSG_DELETE_PUB_BUY_INFO_SUCCESS;
					} else {
						message.what = BuyMessageType.MSG_DELETE_PUB_BUY_INFO_FAILED;
					}
					sendMessage(message);
				}
			}
