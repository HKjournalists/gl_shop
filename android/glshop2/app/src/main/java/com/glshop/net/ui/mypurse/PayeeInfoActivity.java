...
public class PayeeInfoActivity extends BasicActivity {
	...
	private void initData() {
		...
		if (info != null) {
			...
			// 个人身份不显示证明材料
			if (isPersonType()) {
				getView(R.id.item_devider_auth_title).setVisibility(View.GONE);
				getView(R.id.ll_auth_title).setVisibility(View.GONE);
				getView(R.id.item_devider_auth_content).setVisibility(View.GONE);
				getView(R.id.tv_view_certificate_document).setVisibility(View.GONE);
			}
		} else {
			showToast("收款人信息不能为空!");
			finish();
