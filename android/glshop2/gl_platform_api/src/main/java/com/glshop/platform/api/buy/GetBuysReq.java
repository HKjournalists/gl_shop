	...
	protected void parseData(GetBuysResult result, ResultItem item) {
		...
		if (items != null && items.size() > 0) {
			...
			for (ResultItem buyItem : items) {
				...
				info.isApply = buyItem.getInt("isApply") == 1;
				
				ResultItem authItem = (ResultItem) buyItem.get("authstatus");
				if (authItem != null) {
					info.isAuth = authItem.getInt("val") == 1;
				}
				ResultItem ggItem = (ResultItem) buyItem.get("bailstatus");
				if (ggItem != null) {
					info.isGiven = ggItem.getInt("val") == 1;
				}
