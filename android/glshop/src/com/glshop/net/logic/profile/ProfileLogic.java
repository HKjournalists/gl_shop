			...
			public void onReturn(Object invoker, ResponseEvent event, GetAddrListResult result) {
				if (ResponseEvent.isFinish(event)) {
					...
					if (result.isSuccess()) {
						...
						//Add test data
						/*List<AddrInfoModel> data = new ArrayList<AddrInfoModel>();
						for (int i = 0; i < Common.PAGE_SIZE; i++) {
							AddrInfoModel info = new AddrInfoModel();
							info.addrId = "000000" + i;
							info.deliveryAddrDetail = "广东省深圳市南山区蛇口港码头" + i;
							info.uploadPortWaterDepth = 60;
							info.uploadPortShippingWaterDepth = 60;
							data.add(info);
						}
						result.datas = data;*/
						//End add
