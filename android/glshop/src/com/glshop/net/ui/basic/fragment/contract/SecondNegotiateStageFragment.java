...
import com.glshop.net.ui.basic.adapter.contract.SecondNegotiateAdapter;
 ...
 */
public class SecondNegotiateStageFragment extends BaseContractInfoFragment {
	...
	protected void initData() {
		if (mContractInfo != null) {
			...
			List<NegotiateInfoModel> list = mContractInfo.secondNegotiateList;
			NegotiateListView lv = getView(R.id.ll_second_negotiate_list);
			if (BeanUtils.isNotEmpty(list)) {
				SecondNegotiateAdapter adapter = new SecondNegotiateAdapter(mContext, list);
				adapter.setContractInfo(mContractInfo);
	...
	protected void handleStateMessage(Message message) {
		...
		switch (message.what) {
		case ContractMessageType.MSG_CONTRACT_ACCEPTANCE_SUCCESS: // 合同验收或同意第二次议价成功
			onAcceptanceSuccess(respInfo);
			break;
		case ContractMessageType.MSG_CONTRACT_ACCEPTANCE_FAILED: // 合同验收或同意第二次议价失败
