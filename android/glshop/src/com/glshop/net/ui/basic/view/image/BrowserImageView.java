...
public class BrowserImageView extends ImageView {
	...
	public BrowserImageView(Context context, int imageWidth, int imageHeight) {
		super(context);
	...
	public BrowserImageView(Context context, AttributeSet attrs, int imageWidth, int imageHeight) {
		super(context, attrs);
		this.imageHeight = imageHeight;
		this.imageWidth = imageWidth;
		init();
	...
	private void arithScaleRate() {
		float scaleWidth = BrowsePictureActivity.screenWidth / (float) imageWidth;
		float scaleHeight = BrowsePictureActivity.screenHeight / (float) imageHeight;
		scaleRate = Math.min(scaleWidth, scaleHeight);
		if (scaleRate > mMaxZoom) {
			mMaxZoom = scaleRate;
		}
	...
	public void requestImage() {
		...
		arithScaleRate();
		// 缩放到屏幕大小
		zoomTo(scaleRate, BrowsePictureActivity.screenWidth / 2f, BrowsePictureActivity.screenHeight / 2f);
		// 居中
		layoutToCenter();
	}
	...
	public void layoutToCenter() {
		...
		// 空白区域宽高
		float fill_width = BrowsePictureActivity.screenWidth - width;
		float fill_height = BrowsePictureActivity.screenHeight - height;
		...
		tran_width -= left;
		tran_height -= top;
		//Logger.e(TAG, "TranWidth = " + tran_width + ", TranHeight = " + tran_height);
	...
	protected void zoomTo(float scale, float centerX, float centerY) {
		...
		// 图片无需缩放,屏幕足够显示这张图片,取消放大
		//		if (scaleRate == 1f && scale > 1) {
		//			return;
		//		}
		if (scale > mMaxZoom) {
			scale = mMaxZoom;
	...
	protected void zoomTo(final float scale, final float centerX, final float centerY, final float durationMs) {
		...
		mHandler.post(new Runnable() {
			public void run() {
				...
				//Logger.e(TAG, "Target = " + target + ", OldScale = " + oldScale);
