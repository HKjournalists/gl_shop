//
//  ContractPorccesingViewController.m
//  Glshop
//
...
#define kProcessSureTableDateKey @"kProcessSureTableDateKey"

static NSString *const ContractPorccesingViewControllerTitle = @"进行中的合同";

@interface ContractPorccesingViewController () <UITableViewEventDelegate>

@property (nonatomic, strong) ContractTableView *tableView;

@end

@implementation ContractPorccesingViewController
...
- (void)loadSubViews {
    ...
    [_tableView addLegendHeaderWithRefreshingBlock:^{
        [this refresh:YES];
    } dateKey:kProcessSureTableDateKey];
...
- (void)requestSuccessButNoData {
    if (self.shouldShowFailView && !self.failView.superview) {
        [self.view addSubview:[self failViewWithFrame:self.view.bounds expectionImgName:nil expectionTitle:@"暂无进行中合同" expectionSubTitle:nil isNodata:YES]];
    }
}
