...
//                segment.titleLabel.font = [UIFont systemFontOfSize:15.f];
			}
		}
		...
		segment.titleLabel.font = [UIFont systemFontOfSize:16.];
...
- (void)setTintColor:(UIColor *)tintColor {
	...
	_divider.backgroundColor = RGB(240, 240, 240, 1);
}
...
//		}
	...
	path = [UIBezierPath bezierPathWithRoundedRect:self.bounds byRoundingCorners:roundingCorners cornerRadii:CGSizeMake(0, 0)];

	if (self.selected) {
        if (self.enabled) {
//            [self.tintColor setFill];
            if (_selectedStyle == WLSegmentSelectedStyleFill) {
                
                [RGB(252, 187, 52, 1) setFill];
            }else if(_selectedStyle == WLSegmentSelectedStyleFillText){
                [[UIColor clearColor] setFill];
            }else if (_selectedStyle == WLSegmentSelectedStyleSketch) {
                [[UIColor clearColor] setFill];
                UIImage *image = [UIImage imageNamed:@"down_dark0"];
                [image drawInRect:CGRectMake(self.vright-23, self.vheight/2-10, 20, 20)];
            }
