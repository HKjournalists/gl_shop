...
- (void)endRefreshing
{
    [self setTitle:MJRefreshHeaderStateCompleteText forState:MJRefreshHeaderStateIdle];
    self.state = MJRefreshHeaderStateSuccess;
    ...
    switch (state) {
        case MJRefreshHeaderStateIdle: {
           // if (oldState == MJRefreshHeaderStateRefreshing) {
            if (oldState == MJRefreshHeaderStateSuccess) {
                // 保存刷新时间
...
//                } completion:nil];
                // 恢复inset和offset
                [UIView animateWithDuration:MJRefreshSlowAnimationDuration delay:0.5 options:UIViewAnimationOptionAllowUserInteraction|UIViewAnimationOptionBeginFromCurrentState animations:^{
                    // 修复top值不断累加
                    _scrollView.mj_insetT -= self.mj_h;
