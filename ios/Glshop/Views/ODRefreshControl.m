...
- (UILabel *)labelSuccess {
    if (!_labelSuccess) {
        _labelSuccess = [[UILabel alloc] initWithFrame:CGRectMake(0, 400-44, SCREEN_WIDTH, 44)];
        _labelSuccess.backgroundColor = [UIColor clearColor];
        _labelSuccess.text = @"刷新成功";
        _labelSuccess.textAlignment = NSTextAlignmentCenter;
        _labelSuccess.textColor = [UIColor lightGrayColor];
    }
...
- (void)endRefreshing
{
    [self performSelector:@selector(end) withObject:nil afterDelay:0.6];
...
- (void)end {
    if (_refreshing) {
        self.refreshing = NO;
        //        [self addSubview:self.labelSuccess];
