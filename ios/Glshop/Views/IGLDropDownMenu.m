...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        [self resetParams];
        self.menuButton = [[IGLDropDownItem alloc] initWithCustomStyle];
    }
    return self;
}
...
{
    // expand the view
    [self.menuButton rotaionIndicate:NO];
    ...
    for (int i = (int)self.dropDownItems.count - 1; i >= 0; i--) {
        ...
        [UIView animateWithDuration:self.animationDuration delay:0 options:self.animationOption animations:^{
...
{
    // fold the view
    [self.menuButton rotaionIndicate:YES];
...
- (CGRect)frameOnFoldForItemAtIndex:(NSInteger)index
{
    ...
    return CGRectMake(0, 0, self.itemSize.width, self.itemSize.height);
}
