...
public class ContractInfoDAOImpl extends BaseJdbcDao<TOrderInfo> implements
	...
	private RowMapper<ContractInfoBean> rowMapper = new RowMapper<ContractInfoBean>() {
		public ContractInfoBean mapRow(ResultSet rs,
			...
			bean.setSaleType(rs.getString("SALETYPE"));
			...
			bean.setLifecycle(rs.getString("LIFECYCLE"));

			return bean;
		}
	};
	
	private String dynamicJoinSqlWithEntity(TOrderInfo entity,StringBuffer sql){
	...
	public TOrderInfo mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		tOrder.setOtype(rs.getString("OTYPE"));
		tOrder.setSellerid(rs.getString("SELLERID"));
		tOrder.setStatus(rs.getString("STATUS"));
		...
		tOrder.setLifecycle(rs.getString("LIFECYCLE"));
		return tOrder;
	}
	...
	public QueryContext<ContractInfoBean> queryContractInfoListForPagination(
		...
		String cid = qContext.getParameters().get("cid").toString();
		StringBuffer sql = new StringBuffer(" SELECT	CASE toi.SELLERID WHEN '" +cid+ "' THEN	'0'  ELSE 	'1' END AS SALETYPE,topi.PNAME as PRODUCTNAME,sellerTci.CNAME as SELLERNAME, ");
