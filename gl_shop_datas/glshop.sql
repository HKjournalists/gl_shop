...
) ENGINE=InnoDB AUTO_INCREMENT=53 DEFAULT CHARSET=utf8 COMMENT='企业认证记录';
...
CREATE TABLE `T_COMPANY_ADDRESS` (
  ...
  `CID` varchar(50) DEFAULT NULL COMMENT '企业编号',
  `AREACODE` varchar(50) DEFAULT NULL COMMENT '地区编码（只存最后一级编码）',
...
) ENGINE=InnoDB AUTO_INCREMENT=25 DEFAULT CHARSET=utf8 COMMENT='企业联系人';
...
CREATE TABLE `T_COMPANY_EVALUATION` (
  ...
  `CRATEDATE` datetime DEFAULT NULL COMMENT '评价时间',
...
) ENGINE=InnoDB AUTO_INCREMENT=31 DEFAULT CHARSET=utf8 COMMENT='企业评价表';
...
CREATE TABLE `T_COMPANY_PERSONAL` (
  ...
  `ISSUINGAUTH` varchar(250) DEFAULT NULL COMMENT '签发机关',
  `EFFSTARTTIME` datetime DEFAULT NULL COMMENT '有效期限-开始时间',
  `EFFENDTIME` datetime DEFAULT NULL COMMENT '有效期限-结束时间',
  PRIMARY KEY (`CPID`),
  KEY `FK_Reference_au_cp` (`AUTHID`),
  CONSTRAINT `FK_Reference_au_cp` FOREIGN KEY (`AUTHID`) REFERENCES `T_AUTH_RECORD` (`AUTHID`)
) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=utf8 COMMENT='个人认证信息表';
...
CREATE TABLE `T_COMPANY_RANKING` (
  ...
  `CREDIT` float(12,3) DEFAULT NULL COMMENT '交易诚信度',
  PRIMARY KEY (`SID`),
  KEY `FK_Reference_33` (`CID`),
  CONSTRAINT `FK_Reference_33` FOREIGN KEY (`CID`) REFERENCES `T_COMPANY_INFO` (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=utf8 COMMENT='企业统计信息';
...
) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=utf8 COMMENT='企业船舶信息';
...
CREATE TABLE `T_MOBILE_APP_VERSION` (
  ...
  `LASTEST` varchar(50) NOT NULL COMMENT '最新版本号',
...
CREATE TABLE `T_OFFLINE_PAY` (
  ...
  `CREATETIME` date DEFAULT NULL COMMENT '收款时间',
...
) ENGINE=InnoDB AUTO_INCREMENT=78 DEFAULT CHARSET=utf8 COMMENT='订单的收货地址';
...
CREATE TABLE `T_ORDER_FIND` (
  ...
  `CONTRACTID` varchar(50) DEFAULT NULL COMMENT '合同ID',
  PRIMARY KEY (`FID`),
...
) ENGINE=InnoDB AUTO_INCREMENT=15 DEFAULT CHARSET=utf8 COMMENT='感兴趣、客服电话沟通列表';
...
-- ----------------------------
-- Table structure for T_ORDER_MINE
-- ----------------------------
DROP TABLE IF EXISTS `T_ORDER_MINE`;
CREATE TABLE `T_ORDER_MINE` (
  `ID` int(11) NOT NULL AUTO_INCREMENT COMMENT '编号',
  `CID` varchar(50) DEFAULT NULL COMMENT '企业编号',
  `OID` varchar(50) DEFAULT NULL COMMENT '订单编号',
  `STATUS` varchar(20) DEFAULT NULL COMMENT '订单状态(订单的进行中的状态)',
  `LIFECYCLE` varchar(20) DEFAULT NULL COMMENT '生命周期',
  `REMARK` varchar(1000) DEFAULT NULL COMMENT '备注',
  `CREATOR` varchar(50) DEFAULT NULL COMMENT '创建人',
  `CREATETIME` datetime DEFAULT NULL COMMENT '创建时间',
  ...
  `UPDATETIME` datetime DEFAULT NULL COMMENT '修改时间',
  PRIMARY KEY (`ID`),
  KEY `FK_Reference_61` (`OID`),
  KEY `FK_Reference_62` (`CID`),
  CONSTRAINT `FK_Reference_61` FOREIGN KEY (`OID`) REFERENCES `T_ORDER_INFO` (`OID`),
  CONSTRAINT `FK_Reference_62` FOREIGN KEY (`CID`) REFERENCES `T_COMPANY_INFO` (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=60 DEFAULT CHARSET=utf8 COMMENT='我的订单(我的合同)';
...
CREATE TABLE `T_ORDER_OPERATIONS` (
  ...
  `OLDSTATUS` varchar(20) DEFAULT NULL COMMENT '操作时候的合同前一个状态',
  PRIMARY KEY (`LID`),
...
) ENGINE=InnoDB AUTO_INCREMENT=132 DEFAULT CHARSET=utf8 COMMENT='交易中的商品详细';
...
CREATE TABLE `T_ORDER_PRODUCT_PROPERTY` (
  ...
  `PPROID` varchar(50) DEFAULT NULL COMMENT '父商品属性ID（T_PRODUCT_PROPERTY_ID）',
  ...
  `CODE` varchar(30) DEFAULT NULL COMMENT '属性编码',
...
) ENGINE=InnoDB AUTO_INCREMENT=881 DEFAULT CHARSET=utf8 COMMENT='交易中的商品的属性';
...
) ENGINE=InnoDB AUTO_INCREMENT=129 DEFAULT CHARSET=utf8 COMMENT='商品价格明细(当天)';
...
) ENGINE=InnoDB AUTO_INCREMENT=65 DEFAULT CHARSET=utf8 COMMENT='商品价格预测';
...
) ENGINE=InnoDB AUTO_INCREMENT=351 DEFAULT CHARSET=utf8 COMMENT='商品的其他属性';
...
CREATE TABLE `T_PUBLIC_CODES` (
  ...
  `ORDERNO` int(11) DEFAULT NULL COMMENT '序号',
  `ISHIDDEN` int(11) unsigned DEFAULT NULL COMMENT '隐藏(0显示，1隐藏)',
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=13234 DEFAULT CHARSET=utf8 COMMENT='公共代码集表';
...
CREATE TABLE `T_PUSH_RESULT` (
  ...
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=1074 DEFAULT CHARSET=utf8 COMMENT='消息推送结果';
...
CREATE TABLE `T_SCHEDULE_INFO` (
  ...
  `SCH_NAME` varchar(100) DEFAULT NULL COMMENT '调度名称',
  ...
  `CREATEDATE` datetime DEFAULT NULL COMMENT '创建时间',
  PRIMARY KEY (`SCHID`)
) ENGINE=InnoDB AUTO_INCREMENT=219 DEFAULT CHARSET=utf8 COMMENT='定时任务调度信息';
...
CREATE TABLE `T_SHORT_MESSAGE_USED` (
  ...
  `REMARK` varchar(250) DEFAULT NULL COMMENT '备注',
  PRIMARY KEY (`SMUID`)
) ENGINE=InnoDB AUTO_INCREMENT=788 DEFAULT CHARSET=utf8 COMMENT='T_SHORT_MESSAGE_USED\r\n短信接口使用记录';
...
CREATE TABLE `T_SYSTEM_LOG` (
  ...
  PRIMARY KEY (`LOGID`)
) ENGINE=InnoDB AUTO_INCREMENT=189 DEFAULT CHARSET=utf8 COMMENT='系统日志';
...
CREATE TABLE `T_SYSTEM_MESSAGE` (
  ...
  PRIMARY KEY (`MSGID`)
) ENGINE=InnoDB AUTO_INCREMENT=1331 DEFAULT CHARSET=utf8 COMMENT='系统消息';
...
CREATE TABLE `T_SYSTEM_PARAMS` (
  ...
  `UPDATETIME` datetime DEFAULT NULL COMMENT '修改时间',
  PRIMARY KEY (`SID`)
) ENGINE=InnoDB AUTO_INCREMENT=29 DEFAULT CHARSET=utf8 COMMENT='系统参数';
...
CREATE TABLE `T_UPLOAD_IMAGES` (
  ...
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=3727 DEFAULT CHARSET=utf8 COMMENT='图片上传管理表';
...
) ENGINE=InnoDB AUTO_INCREMENT=177 DEFAULT CHARSET=utf8 COMMENT='用户通行证';

-- ----------------------------
-- View structure for ORDERINFOSELLERBUYERLASTOPERATOR
-- ----------------------------
DROP VIEW IF EXISTS `ORDERINFOSELLERBUYERLASTOPERATOR`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `ORDERINFOSELLERBUYERLASTOPERATOR` AS select `BB`.`LID` AS `BLID`,`BB`.`OPERATOR` AS `BOPERATOR`,`BB`.`OPERATIONTIME` AS `BOPERATIONTIME`,`BB`.`TYPE` AS `BTYPE`,`BB`.`ORDERSTATUS` AS `BORDERSTATUS`,`BB`.`OLDSTATUS` AS `BOLDSTATUS`,`SS`.`LID` AS `SLID`,`SS`.`OPERATOR` AS `SOPERATOR`,`SS`.`OPERATIONTIME` AS `SOPERATIONTIME`,`SS`.`TYPE` AS `STYPE`,`SS`.`ORDERSTATUS` AS `SORDERSTATUS`,`SS`.`OLDSTATUS` AS `SOLDSTATUS`,`A`.`OID` AS `OID`,`A`.`FID` AS `FID`,`A`.`SELLERID` AS `SELLERID`,`A`.`BUYERID` AS `BUYERID`,`A`.`PRICE` AS `PRICE`,`A`.`TOTALNUM` AS `TOTALNUM`,`A`.`CREATIME` AS `CREATIME`,`A`.`CREATER` AS `CREATER`,`A`.`LIMITTIME` AS `LIMITTIME`,`A`.`TOTALAMOUNT` AS `TOTALAMOUNT`,`A`.`AMOUNT` AS `AMOUNT`,`A`.`STATUS` AS `STATUS`,`A`.`OTYPE` AS `OTYPE`,`A`.`REMARK` AS `REMARK`,`A`.`UPDATER` AS `UPDATER`,`A`.`UPDATETIME` AS `UPDATETIME`,`A`.`LIFECYCLE` AS `LIFECYCLE` from ((`T_ORDER_INFO` `A` left join `ORDEROPERGROUPINFO` `BB` on(((`BB`.`OPERATOR` = `A`.`BUYERID`) and (`A`.`OID` = `BB`.`OID`)))) left join `ORDEROPERGROUPINFO` `SS` on(((`SS`.`OPERATOR` = `A`.`SELLERID`) and (`A`.`OID` = `SS`.`OID`)))) ;

-- ----------------------------
-- View structure for ORDEROPERGROUPINFO
-- ----------------------------
DROP VIEW IF EXISTS `ORDEROPERGROUPINFO`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `ORDEROPERGROUPINFO` AS select `B`.`LID` AS `LID`,`B`.`OID` AS `OID`,`B`.`OPERATOR` AS `OPERATOR`,`B`.`OPERATIONTIME` AS `OPERATIONTIME`,`B`.`TYPE` AS `TYPE`,`B`.`RESULT` AS `RESULT`,`B`.`PLID` AS `PLID`,`B`.`REMARK` AS `REMARK`,`B`.`ORDERSTATUS` AS `ORDERSTATUS`,`B`.`OLDSTATUS` AS `OLDSTATUS` from (`ORDEROPERGROUPOID` `A` left join `T_ORDER_OPERATIONS` `B` on(((`A`.`OPERATOR` = `B`.`OPERATOR`) and (`B`.`LID` = `A`.`OPERS`) and (`A`.`OID` = `B`.`OID`)))) ;

-- ----------------------------
-- View structure for ORDEROPERGROUPOID
-- ----------------------------
DROP VIEW IF EXISTS `ORDEROPERGROUPOID`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `ORDEROPERGROUPOID` AS select `B`.`OID` AS `OID`,`B`.`OPERATOR` AS `OPERATOR`,max(`B`.`LID`) AS `OPERS` from `T_ORDER_OPERATIONS` `B` group by `B`.`OPERATOR`,`B`.`OID` ;


DROP VIEW ORDEROPERGROUPOID;
	CREATE VIEW ORDEROPERGROUPOID AS 
	SELECT B.OID,OPERATOR,MAX(LID) OPERS FROM T_ORDER_OPERATIONS B GROUP BY B.OPERATOR,B.OID
	
DROP VIEW ORDEROPERGROUPINFO;
	CREATE VIEW ORDEROPERGROUPINFO AS 
	SELECT B.* FROM ORDEROPERGROUPOID A	LEFT JOIN T_ORDER_OPERATIONS  B ON A.OPERATOR = B.OPERATOR AND B.LID = A.OPERS AND A.OID = B.OID 

DROP VIEW ORDERINFOSELLERBUYERLASTOPERATOR;	
	CREATE VIEW ORDERINFOSELLERBUYERLASTOPERATOR AS SELECT 
	BB.LID AS BLID,BB.OPERATOR AS BOPERATOR,BB.OPERATIONTIME AS BOPERATIONTIME,BB.TYPE AS BTYPE,BB.ORDERSTATUS AS BORDERSTATUS,BB.OLDSTATUS AS BOLDSTATUS,
	SS.LID AS SLID,SS.OPERATOR AS SOPERATOR,SS.OPERATIONTIME AS SOPERATIONTIME,SS.TYPE AS STYPE,SS.ORDERSTATUS AS SORDERSTATUS,SS.OLDSTATUS AS SOLDSTATUS,
	A.* FROM T_ORDER_INFO A
	LEFT JOIN ORDEROPERGROUPINFO BB ON BB.OPERATOR = A.BUYERID AND A.OID = BB.OID 
	LEFT JOIN ORDEROPERGROUPINFO SS ON SS.OPERATOR = A.SELLERID AND A.OID = SS.OID 

	
CREATE INDEX T_ORDER_OPERATIONS_IndexName ON T_ORDER_OPERATIONS (OID,OPERATOR,LID);
