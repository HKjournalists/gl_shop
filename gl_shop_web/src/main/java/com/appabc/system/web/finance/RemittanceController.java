...
import com.appabc.datas.system.FinanceService;
...
public class RemittanceController extends FinanceController {
    ...
    public String doInput(@ModelAttribute("remittance") Remittance remittance, BindingResult result) {
        // TODO: need to validate input
        Remittance r = new Remittance();
        BeanUtils.copyProperties(remittance, r);
        if (!StringUtils.hasText(r.getBankSerialNumber())) {
            SimpleDateFormat sfmt = new SimpleDateFormat("yyyyMMddHHmm");
            r.setBankSerialNumber(sfmt.format(new Date()));
        }
        r.setStatus(Remittance.Status.PROCESSING);
        r.setCreator(getCurrentUser());
    ...
    public String findPossibleTarget(@PathVariable int id, ModelMap model) {
        ...
        // Find by contract id
        if (StringUtils.hasText(r.getContractId())) {
            TOrderInfo order = contractInfoService.query(r.getContractId().trim());
            if (order != null) {
                TCompanyInfo qCompany = new TCompanyInfo();
                qCompany.setId(order.getBuyerid());
                findCompany(qCompany, possibleTargets);
                qCompany.setId(order.getSellerid());
                findCompany(qCompany, possibleTargets);
            }
        }
    ...
    private void showMoreInfoOfRemit(ModelMap model, Remittance r) {
        if (StringUtils.hasText(r.getMobile()) || StringUtils.hasText(r.getCompany()) || StringUtils.hasText(r.getContractId())
    ...
    private void findCompanyByAcceptBank(TAcceptBank qab, Set<TCompanyInfo> possibleTargets) {
        ...
        if (!CollectionUtils.isEmpty(acceptBanks)) {
            for (TAcceptBank acceptBank : acceptBanks) {
                if (acceptBank.getAuthstatus() == AcceptBankInfo.AcceptAuthStatus.AUTH_STATUS_CHECK_YES) {
    ...
    private void findCompany(TCompanyInfo q, Set<TCompanyInfo> possibleTargets) {
        ...
        if (!CollectionUtils.isEmpty(companies)) {
            for (TCompanyInfo company : companies) {
                if (company.getAuthstatus() == CompanyInfo.CompanyAuthStatus.AUTH_STATUS_YES) {
