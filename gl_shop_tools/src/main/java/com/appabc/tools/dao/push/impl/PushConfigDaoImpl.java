 ...
 */
@Repository
public class PushConfigDaoImpl extends BaseJdbcDao<TPushConfig> implements IPushConfigDao {
	...
	private static final String UPDATESQL = " update T_PUSH_CONFIG set APPID = :appid, APPKEY = :appkey, APPSECRET = :appsecret, MASTERSECRET = :mastersecret, URL = :url, STATUS = :status, TYPE = :type, UPDATER = :updater, UPDATETIIME = :updatetiime, PORT=:port, CERTIFICATEPATH=:certificatepathwhere ID = :id ";
	...
	public void delete(TPushConfig entity) {
		super.delete(dynamicJoinSqlWithEntity(entity,  new StringBuilder(BASE_SQL)), entity);
	}
	...
	public List<TPushConfig> queryForList(Map<String, ?> args) {
		return queryForList(BASE_SQL, args);
	}
	...
	public TPushConfig mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		t.setStatus(ConfigStatusEnum.enumOf(rs.getInt("STATUS")));
		t.setType(ConfigTypeEnum.enumOf(rs.getString("TYPE")));
	...
	private String dynamicJoinSqlWithEntity(TPushConfig bean,StringBuilder sql){
